package example.operation.impl.studyArticle;
...
public class studyOpt {
    private static Logger logger = LoggerFactory.getLogger(studyOpt.class);
     ...
     * @param msg http传递的数据
     */
    public static ResponseData getRangeStudyListToBg(Object msg) {
        ResponseData responseData = new ResponseData(StatusCode.ERROR.getValue());
        SqlSession sqlSession = MybatisUtils.getSession();
        String message = "";
        try {
            Map<String, Object> map = FormData.getParam(msg);
            List<DynamicInfo> newsList = sqlSession.selectList(Mapper.GET_RANGE_STUDY_TO_BG, map);
            int newsNum = sqlSession.selectOne(Mapper.GET_STUDY_NUM);
            //检查是否查找到指定起始位置及数目的新闻并返回相应结果
            if (CommonService.checkNotNull(newsList)) {
                //设置回传的返回数据
                Map<String, Object> data = new HashMap<String, Object>(2);
                data.put(Common.TOTAL_NUM, newsNum);
                data.put(Common.STUDY_LIST_DATA, newsList);
                Assemble.responseSuccessSetting(responseData, data);

            } else {
                message = "news info not found";
                studyOpt.logger.warn(message);
                Assemble.responseErrorSetting(responseData, 401, message);
            }

        } catch (Exception e) {
            message = "sys error";
            studyOpt.logger.debug(message, e);
     ...
     * @return
     */
    public static ResponseData searchStudyData(Object msg) {
        return CommonService.simpleImplOpt(false, (responseData, sqlSession) -> {
            Map<String, Object> map = FormData.getParam(msg);
            List<DynamicInfo> list = sqlSession.selectList(Mapper.SEARCH_STUDY_DATA, map);
            int newsNum = sqlSession.selectOne(Mapper.SEARCH_STUDY_DATA_NUM, map);
            Map<String, Object> data = new HashMap<String, Object>(2);
            data.put(Common.TOTAL_NUM, newsNum);
            data.put(Common.STUDY_LIST_DATA, list);
            Assemble.responseSuccessSetting(responseData, data);
     ...
     * @return
     */
    public static ResponseData saveStudyData(Object msg) {
        ResponseData responseData = new ResponseData(StatusCode.ERROR.getValue());
        SqlSession sqlSession = MybatisUtils.getSession();
        String message = "";
        int num = 0;
        try {
            ...
            studyOpt.logger.error(message, e);
            Assemble.responseErrorSetting(responseData, 500, message);
    ...
    private static void saveHtmlFile(DynamicInfo dynamicInfo) throws Exception {
        ...
        try {
            ...
            studyOpt.logger.debug("final html file name: " + systemFileName);
     ...
     * @param msg
     */
    public static ResponseData deleteStudy(Object msg) {
        ResponseData responseData = new ResponseData(StatusCode.ERROR.getValue());
        SqlSession sqlSession = MybatisUtils.getSession();
        String message = "";
     ...
     * @param msg
     */
    public static ResponseData deleteBatchStudy(Object msg) {
        ResponseData responseData = new ResponseData(StatusCode.ERROR.getValue());
        SqlSession sqlSession = MybatisUtils.getSession();
        String message = "";
        try {
     ...
     * 删除趣文数据操作
     */
    public static void deleteStudyOpt(SqlSession sqlSession, Map<String, Object> map) throws Exception {
        //删除数据库数据
     ...
     * @param msg
     */
    public static ResponseData copyStudy(Object msg) {
        ResponseData responseData = new ResponseData(StatusCode.ERROR.getValue());
        SqlSession sqlSession = MybatisUtils.getSession();
        String message = "";
